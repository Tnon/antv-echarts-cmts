...
import type { InitFlyLine } from './tools/flyLine'
...
class Earth {
  ...
  load = () => {
    ...
    const { object3D, waveMeshArr, flyManager } = earth3dObj(this.cityList, this.flyLineData)
  ...
  init() {
    ...
    const orbitControl = new OrbitControls(this.camera, this.renderer2d.domElement)
  ...
  autoRotateEarth() {
    ...
    //旋转地球动画
    const rotateEarthStep = new TWEEN.Tween({
    ...
    const rotateEarthStepBack = new TWEEN.Tween({
      rotateY: endRotateY,
      zoom: endZoom
    })
