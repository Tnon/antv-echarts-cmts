...
export const earth3dObj = (cityList?: Record<string, City>, flyLineData?: FlyData[]) => {
  const object3D = new Object3D()
  const earthMesh = createEarthImageMesh(GlobalConfig.earthRadius)
  // let cloudMesh = createEarthCloudImageMesh(earthRadius + 0.03)
  // object3D.add(cloudMesh);
  const glow = earthGlow(GlobalConfig.earthRadius, earthGlowPng, 3.05)
  const glowLight = earthGlow(GlobalConfig.earthRadius, earthGlowLightPng, 3.15)
  ...
  if (cityList && flyLineData) {
    const { waveMeshArr, pointMeshArr } = getCityMeshGroup(cityList)
    ...
    //添加飞线
    const flyManager = earthAddFlyLine(object3D, flyLineData, cityList)
