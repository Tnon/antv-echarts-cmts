...
import { PlaneGeometry, TextureLoader, MeshBasicMaterial, DoubleSide, Mesh, Vector3 } from 'three'
...
export const getCityMeshGroup = (cityList: Record<string, City>) => {
  ...
  for (const cityName in cityList) {
    ...
    const cityXyz = lon2xyz(GlobalConfig.earthRadius, city.longitude, city.latitude)
    ...
    const size1 = GlobalConfig.earthRadius * 0.05 //矩形平面Mesh的尺寸
    cityMesh.scale.set(size1, size1, size1) //设置mesh大小

    const size2 = GlobalConfig.earthRadius * 0.12 //矩形平面Mesh的尺寸
    ;(cityWaveMesh as any).size = size2 //自顶一个属性，表示mesh静态大小
    cityWaveMesh.scale.set(size2, size2, size2) //设置mesh大小
    ...
    const coordVec3 = new Vector3(cityXyz.x, cityXyz.y, cityXyz.z).normalize()
